...
export class AuthService {
  ...
  login2(emailid: string, password: string){
    ...
    this.Logstatus = true
    console.log(emailid+""+password);
    return this.http.post(`http://localhost:8084/loginDoctor`, {
      emailid,
      password
    }, httpOptions);
  }
  ...
  store2(pid:number,resources:string,classmssg:string): Observable<any> {
     console.log(pid);
    return this.http.post('http://localhost:8084/addpres', {
     pid, 
     resources,
     classmssg
    }, {responseType: 'text'});
